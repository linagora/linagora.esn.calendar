...
describe('The calEventService service', function() {
  ...
  describe('The acceptChanges function', function() {
    var event, suggestedEvent;

    beforeEach(function() {
      var vcalendar = new ICAL.Component('vcalendar');
      var vevent = new ICAL.Component('vevent');

      vevent.addPropertyWithValue('uid', '123');
      vevent.addPropertyWithValue('summary', 'Test acceptChanges');
      vevent.addPropertyWithValue('dtstart', dtstart);
      vevent.addPropertyWithValue('dtend', dtend);
      vevent.addPropertyWithValue('transp', 'OPAQUE');
      vcalendar.addSubcomponent(vevent);
      ...
      event = new self.CalendarShell(self.vcalendar, {
        path: '/path/to/uid.ics'
      });

      suggestedEvent = event.clone();
      suggestedEvent.start = event.start.clone().add(1, 'hour');
      suggestedEvent.end = event.end.clone().add(1, 'hour');
    });

    it('should reject when fieldsToChange is not defined', function(done) {
      self.calEventService.acceptChanges().then(function() {
        done(new Error('Should not occur'));
      }, function(err) {
        expect(err.message).to.match(/fields to change must be specified/);
        done();
      });
    ...
    it('should reject when fieldsToChange is empty', function(done) {
      self.calEventService.acceptChanges(null, null, null, null, []).then(function() {
        done(new Error('Should not occur'));
      }, function(err) {
        expect(err.message).to.match(/fields to change must be specified/);
        done();
    ...
    it('should update the initial event with specified fields from suggested event', function(done) {
      var fields = ['start', 'end'];
      var path = '/event/123.ics';
      var etag = 'etag-test';

      self.calEventService.modifyEvent = sinon.stub().returns($q.when());
      self.calEventService.acceptChanges(path, event, suggestedEvent, etag, fields).then(function() {
        expect(self.calEventService.modifyEvent).to.have.been.calledWith(path, sinon.match(function(updatedEvent) {
          return updatedEvent.start.isSame(suggestedEvent.start) && updatedEvent.end.isSame(suggestedEvent.end);
        }), event, etag);
        done();
      }, done);
