const emailTemplateName = 'resource.declined';
const subject = 'The reservation of resource {{name}} has been declined';
const jcalHelper = require('../../helpers/jcal');

// A resource reservation has been declined by an administrator
...
module.exports = dependencies => {
  ...
  function handle({ics, resourceId, eventPath}) {
    logger.debug('An event in the calendar resource has been declined', eventPath);
    ...
    return resourceModule.lib.resource.get(resourceId).then(resource => {
      ...
      return getEventCreator()
        ...
        .catch(err => {
          logger.error('Error while sending email to event creator', err);
          throw err;
        });
    });
